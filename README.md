...
`fp-analysis.mjs` imports the production detector modules and calls `runPredictiveRules`, the same entry point the browser uses. Its only deviation from the browser path is HTTP transport: `fetch` is shimmed to apply the rewrites that `vite.config.ts` declares, since the app fetches through relative proxy paths that do not resolve under Node. An earlier version of this script reimplemented two detectors inline and produced numbers that described the reimplementation rather than the system. That is why the import path is load-bearing and should not be replaced with a copy.  <!-- claim-verified: a subset that does something, not a count of the population -->
