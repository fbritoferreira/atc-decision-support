 ...
 * and re-derived by the claim checker on any checkout holding the dumps. An
 * earlier version of this comment said 45 of 46, which came from a window that
 * no longer exists here and reproduces from nothing; the papers carry the same
 * figure with the same caveat, and it is quoted here rather than deleted
 * because a comment that silently drops a number leaves the next reader
 * wondering which of the two they misremembered. The deployed analogue behaves the same way: ATPA is
 ...
 *
 * On live data this doctrine fires, which three places used to say it could not,
 * including the previous version of this comment. Ingest synthesises fuel from
